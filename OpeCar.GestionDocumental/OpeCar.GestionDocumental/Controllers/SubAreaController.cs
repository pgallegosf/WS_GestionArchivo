...
namespace OpeCar.GestionDocumental.Controllers
{
    ...
    {
        [HttpGet]
        [Route("api/SubArea/Listar/{idUsuario}/{idArea}")]
        public IHttpActionResult Listar(int? idUsuario,int idArea)
        {
            var resul = DSubArea.Listar(idUsuario,idArea);
            return Ok(resul);
        }
        ...
        [HttpPost]
        [Route("api/SubArea/Mover")]
        public IHttpActionResult Mover([FromBody] ESubAreaRequest request)
        {
            var resul = DSubArea.Mover(request);
            return Ok(resul);
        }
    }
}
